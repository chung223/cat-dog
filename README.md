...
盤面被切成幾個顏色區塊，要在每個區塊裡放滿主角：

- **每個顏色區塊放滿** — 一顆模式每區一個，兩顆模式每區兩個
- **每列每欄一樣多** — 橫的直的都要放滿同樣的數量
- **八個方向不相鄰** — 不能上下左右或斜角貼在一起（兩顆模式裡，同一列的那兩個也不行）
...
- **闖關（⭐ 一顆）** — 100 關，難度照「推理技巧」排（見下），記錄最佳時間，零提示零失誤拿 ⭐
- **闖關（⭐⭐ 兩顆）** — 30 關，每列每欄每區都要放兩個。完全不同的難度量級，見下
...
| 4 | 配對不成立 | 這格放下去，同一單位剩下的就湊不出完整一組了（**只有兩顆模式用得到**） |
| 5 | 集合配對 | k 個區塊還要放的總數，剛好等於它們能用的那幾列還剩的空位總數 |
| 6 | 假設反證 | 假設放這裡 → 推到矛盾 → 不能放 |

一題的難度＝解題器被迫用到的最高階。100 關就照「階級優先、尺寸其次」排，所以難度不會倒退（舊版第 80 關是第 4 階、第 100 關卻只有第 3 階，就是因為只看尺寸）。

## 兩顆模式

每列、每欄、每個顏色區塊都要放**兩個**——這是 Star Battle 標準的升級方式，而且不是「同樣的題目變大張」，是另一種推理。

實測下來三件事很有說服力：

- **8×8 以下根本放不下。** 兩個要保持距離，6×6 和 7×7 連一組合法擺法都不存在，所以兩顆模式從 8×8 起跳。
- **一顆模式用的那套技巧不夠用。** 剛接上時，每一題都被評為最高階，而且有 20～35% 連解都解不出來（需要真的用猜的）。缺的是一招一顆模式根本用不到的推理：**「這格放下去，這一列的第二個就沒地方擺了」**（上表第 4 階）。補上之後解不出來的比例降到 5～15%，那些直接丟掉不出題。
- **階級對兩顆模式沒有鑑別度**，幾乎每題都要用到假設反證。所以兩顆模式的 30 關不是照階級排，是照**需要幾輪假設反證**排的。

出題那邊也要重寫：一顆模式是「每個區塊塞一個種子再往外長」，兩顆模式得先把 2n 個主角**兩兩配對**，再幫每一對挖一條走廊接起來，最後才隨機填滿——不然沒辦法保證每個區塊剛好兩個而且連通。

修唯一解的迴圈也撞牆了：原本一次殺一組雜解，在兩顆模式的解空間裡有題目要修 1900 次，一題要 50 秒。改成一次抓一批雜解、挑「最多解共用的那一格」下手，而且一次搜尋餵好幾步修正，10×10 才降到中位數 270ms。

## 難度分佈

一顆模式：第 1 階 10 關、第 2 階 14 關、第 3 階 24 關、第 5 階 26 關、第 6 階 26 關。
